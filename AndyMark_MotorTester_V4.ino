...
//http://playground.arduino.cc/Code/Button
Button mode_select_button = Button(MODE_SELECT_SWITCH,PULLUP);
Button mode_increment_button = Button(MODE_INCREMENT_SWITCH,PULLUP);
//http://playground.arduino.cc/Code/Potentiometer
...
void set_speed_requested()
{
	...
	if (select_is_pressed())
	{
		while( !select_is_pressed() )
		{
			...
			if ( glb_byte_present_pwm_setting  > read_potentiometer(POT_VALUE_IS_SERVO_POSITION) && glb_byte_present_pwm_setting > glb_servo_start )
			{
				glb_byte_present_pwm_setting--;
			}
			if ( glb_byte_present_pwm_setting  < read_potentiometer(POT_VALUE_IS_SERVO_POSITION) && glb_byte_present_pwm_setting < glb_servo_end )
			{
				glb_byte_present_pwm_setting++;
			}
			...
			if ( bool_update_screen )
			{
				...
				if (int_present_pwm_setting_in_us < 1000)  SSD1306_display.print(F(" "));
				//         else if (glb_int_present_pwm_setting >= 10 && glb_int_present_pwm_setting < 100) SSD1306_display.print(F("   "));
				else ;//                                      SSD1306_display.print(F(""));
				...
				SSD1306_display.fillRect( 7 +  strlen("I  =") * CHAR_WIDTH_FOR_FONTSIZE2, (CHAR_HEIGHT_IN_PIXELS_FOR_FONTSIZE2 + SPEED_STARTING_ROW), (5 * CHAR_WIDTH_FOR_FONTSIZE2), CHAR_HEIGHT_IN_PIXELS_FOR_FONTSIZE2, BLACK );
				
				//Keep value right justified
				if (temp_current_holder < 10.0)                                                    SSD1306_display.print(F(" "));
				else if (temp_current_holder >= 10.0 && temp_current_holder < 100)                   SSD1306_display.print(F(""));
				SSD1306_display.print(temp_current_holder,1);
				
				if (glb_RPM_Value > 0)
				{
					SSD1306_display.setCursor( 3 +  strlen("SPD=") * CHAR_WIDTH_FOR_FONTSIZE2, (( CHAR_HEIGHT_IN_PIXELS_FOR_FONTSIZE2 * 2 ) + SPEED_STARTING_ROW) );
					SSD1306_display.fillRect( 3 +  strlen("SPD=") * CHAR_WIDTH_FOR_FONTSIZE2, (( CHAR_HEIGHT_IN_PIXELS_FOR_FONTSIZE2 * 2 ) + SPEED_STARTING_ROW), (4 * CHAR_WIDTH_FOR_FONTSIZE2), CHAR_HEIGHT_IN_PIXELS_FOR_FONTSIZE2, BLACK );

					//Keep value right justified
					//If value is one digit
					if (glb_RPM_Value < 10)                                                          SSD1306_display.print(F("   "));
					//if value is 2 digits
					else if (glb_RPM_Value >= 10 && glb_RPM_Value < 100)                             SSD1306_display.print(F("  "));
					//if value is 3 digits
					else if (glb_RPM_Value >= 100 && glb_RPM_Value < 1000)                           SSD1306_display.print(F(" "));
					else ;// SSD1306_display.print(F(""));
...
int read_potentiometer(boolean bool_pot_value_is_servo_position)
{
	...
	if (bool_pot_value_is_servo_position)
	{
		...
		byte_pot_value_scaled = map(Speed_Pot.getValue(), 0, 1023, glb_servo_end, glb_servo_start);
...
void display_pwm(String function_name_string)
{
	...
	SSD1306_display.print(map(glb_byte_present_pwm_setting, 0, 180, ARDUINO_SERVO_START, ARDUINO_SERVO_END));
	//if (glb_byte_present_pwm_setting < 10)                                             SSD1306_display.print(F("  "));
	//else if (glb_byte_present_pwm_setting >= 10 && glb_byte_present_pwm_setting < 100) SSD1306_display.print(F(" "));
	//else if (glb_byte_present_pwm_setting >= 100 )                                     SSD1306_display.print(F(""));
